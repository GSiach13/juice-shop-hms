...
describe('/rest/products/search', () => {
  ...
  it('GET product search fails with error message that exposes SQL Injection vulnerability is fixed', () => {
    return frisby.get(`${REST_URL}/products/search?q=';`)
      .expect('status', 400)
      .expect('header', 'content-type', /application\/json/)
      .expect('json', 'error', 'Invalid search query')
  })

  it('GET product search SQL Injection fails from two missing closing parenthesis is fixed', () => {
  ...
  it('GET product search SQL Injection fails from one missing closing parenthesis is fixed', () => {
    return frisby.get(`${REST_URL}/products/search?q=') union select id,email,password from users--`)
      .expect('status', 400)
      .expect('header', 'content-type', /application\/json/)
      .expect('json', 'error', 'Invalid search query')
  ...
  it('GET product search SQL Injection fails for SELECT * FROM attack due to wrong number of returned columns is fixed', () => {
  ...
  it('GET product search can create UNION SELECT with Users table and fixed columns is fixed', () => {
  ...
  it('GET product search can create UNION SELECT with Users table and required columns is fixed', () => {
  ...
  it('GET product search can create UNION SELECT with sqlite_master table and required column is fixed', () => {
